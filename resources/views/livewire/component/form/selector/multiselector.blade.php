...
<div class="w-full flex flex-wrap" x-data="{open : false}">
    <div class="w-full">
        <div class="formField h-24 bg-white flex flex-wrap rounded-t border border-gray-500 overflow-y-auto items-start py-2" 
            ...
            <div class="w-full cursor-text flex flex-wrap content-start"  @click="open = true ;$nextTick(() => $refs.input.focus());" @click.away="open = false">
                @if($selectedItem)
                    @foreach($selectedItem as $keyindex => $slcItem)
                    <p class="flex-none ml-2 px-2 py-1 bg-white border border-gray-500 text-gray-800 rounded mr-2 text-xs">{{$slcItem['label']}} <span class="cursor-pointer ml-2 text-xs text-gray-500 hover:text-gray-700" wire:click="deleteItem({{$keyindex}})">x</span></p>
                    @endforeach
                @endif
