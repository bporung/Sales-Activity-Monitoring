<?php

namespace App\Http\Livewire\Interaction\Method;

use App\Http\Livewire\Component\Modal\Modal;
use Auth;
use Illuminate\Support\Facades\Validator;
// use App\Http\Traits\QuotationTrait;
class Creatingquotation extends Modal
{
    // use QuotationTrait;
    public $data_id;
    public $customer_id;
    public $state = [];

    public function submitData(){
        $this->data_id = isset($this->result) && $this->result && $this->result['id'] ? $this->result['id'] : '';
        $this->customer_id = isset($this->result) && $this->result && $this->result['customer_id'] ? $this->result['customer_id'] : '';

        $validatedData = Validator::make(
            $this->state,
            [
                'description' => 'nullable',
                'due_date' => 'required',
                'term_condition' => 'required',
            ],
            [
                'required' => ':attribute tidak boleh kosong',
            ],
            [
                'description' => 'Description',
                'due_date' => 'Due Date',
                'term_condition' => 'Terms and Conditions',
            ],
        )->validate();
        $interaction = 'App\Models\Interaction'::findOrFail($this->data_id);
        
        // $res = $this->createQuotation($interaction->id,$this->state)->getData();
        // if($res && $res->status === 200){
        //     session()->flash('success', $res->message);
        //     if($res->redirect_link){
        //         return redirect()->to($res->redirect_link);
        //     }
        // }else{
        //     session()->flash('error','There is some error occured');
        //     if($res->redirect_link){
        //         return redirect()->to($res->redirect_link);
        //     }
        // }
    }
    public function render()
    {
        return view('livewire.interaction.method.creatingquotation');
    }
}
